...
import { fieldProfitabilityInputSchema } from "@/lib/field-profitability";
...
export async function createOrganization(formData: FormData) {
  ...
  if (error) {
    console.error(JSON.stringify({ event: "organization_create_failed", code: error.code }));
    throw new Error("Could not create farm organization.");
  }
  revalidatePath("/");
}
...
export async function saveFieldProfitability(formData: FormData) {
  ...
  const input = fieldProfitabilityInputSchema.parse({
  ...
  if (error) {
    console.error(JSON.stringify({
      event: "field_profitability_save_failed",
      organizationId: input.organizationId,
      code: error.code
    }));
    throw new Error("Could not save field profitability record.");
  }
